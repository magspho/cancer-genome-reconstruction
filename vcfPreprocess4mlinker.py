...
def get_readkey(col2idx, words):
    '''find column that store the read sample genotypes'''
    readkey = None
    for key, value in col2idx.items():
        ...
        if flag:
            print(key,value)
            readkey = key
    # if still not found colName with keywords, take first sample (col after FORMAT)
    if not flag:
        keylist = list(col2idx.keys())
        readkey = keylist[keylist.index('FORMAT')+1]
        print(f'Read sample name: {readkey}')
    return readkey
            

def read_vcf(file_name, ref_bp = 1, alt_num = 1, qual_thres = 3, depth_thres = 10, bed_file=None, out_file = None):
    '''
    ...
    for line in open(file_name, "r"):
        ...
        if line_count == 0 and line.startswith("#CHROM"):
            col2idx = {}
            # We want to save everything in a standard vcf file format
...
if len(argv) == 3:
    bed_file = argv[2]
vcf_file = argv[1]
vcf_file_processed = vcf_file.split('/')[-1]

out_file_name = f"processed.{vcf_file_processed}"
